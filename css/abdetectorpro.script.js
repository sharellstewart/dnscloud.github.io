...
(function (root, factory) {
   ...
   * @returns true|false
  */
  const performBasicScriptTest = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // console.error(`Resource not found: ${testUrl}`);
        return false;
      } else {
        return true;
      }
  ...
  const performAdvScriptTest = () => {
    ...
    return new Promise((resolve) => {
      fetch(REQUEST_URL, REQUEST_CONFIG)
          .then((response) => {
              if (response.redirected) { // ads are blocked if request is redirected
                  resolve(true);
              } else if (response.status === 404) { // not found
                  resolve(false);
              } else {
                  resolve(false); // any other status, we assume ads are not blocked
              }
          })
          .catch(() => { // catch fetch errors
              resolve(true); // ads are blocked if request fails
          });
    });
  };
...
*/
  ...
  // Define the localized text configurations for supported languages.
  const localizedTexts = {
    'de': { // Deutsch
      ...
      screenTwoSteps: [
        'Klicken Sie auf das AdBlocker-Symbol oben rechts in Ihrem Browser',
        'Wählen Sie im Menü "Auf dieser Seite deaktivieren" oder "Auf dieser Seite pausieren"',
        'Aktualisieren Sie die Seite, falls sie nicht automatisch aktualisiert wird'
      ]
    },
    'it': { // Italiano
      titleText: "AdBlocker rilevato!",
      descText: "Sembra che tu abbia un adblocker abilitato. Per supportare il nostro sito web e continuare ad accedere ai nostri contenuti, disabilita il tuo ad blocker per questo sito.",
      btnText: "Ho disattivato AdBlock",
      screenOneHelpText: "Come faccio a disabilitare il mio adblocker?",
      screenTwoHelpText: "Va bene, ho capito!",
      screenTwoTitle: "Per disabilitare l'adblocker su questo sito:",
      screenTwoSteps: [
        'Clicca sull’icona dell’adblocker situata in alto a destra nel tuo browser',
        'Dal menu scegli "Disabilita su questo sito" o "Metti in pausa su questo sito"',
        'Aggiorna la pagina se non viene aggiornata automaticamente'
      ]
    },
    'fr': { // Français
      titleText: "AdBlocker détecté!",
      descText: "Il semble que vous ayez un adblocker activé. Pour soutenir notre site Web et continuer à accéder à nos contenus, veuillez désactiver votre bloqueur de publicités pour ce site.",
      btnText: "J'ai désactivé AdBlock",
      screenOneHelpText: "Comment désactiver mon adblocker?",
      screenTwoHelpText: "D'accord, je comprends!",
      screenTwoTitle: "Pour désactiver l'adblocker sur ce site:",
      screenTwoSteps: [
        'Cliquez sur l’icône de l’adblocker en haut à droite de votre navigateur',
        'Dans le menu, choisissez "Désactiver sur ce site" ou "Mettre en pause sur ce site"',
        'Rafraîchissez la page si elle n’est pas automatiquement rafraîchie'
      ]
    },
    'es': { // Español
      titleText: "¡Se detectó AdBlocker!",
      descText: "Parece que tienes un adblocker habilitado. Para apoyar nuestro sitio web y seguir accediendo a nuestros contenidos, desactiva tu ad blocker para este sitio.",
      btnText: "He desactivado AdBlock",
      screenOneHelpText: "¿Cómo desactivo mi adblocker?",
      screenTwoHelpText: "¡Vale, lo entiendo!",
      screenTwoTitle: "Para desactivar el adblocker en este sitio:",
      screenTwoSteps: [
        'Haz clic en el icono del adblocker en la parte superior derecha de tu navegador',
        'Desde el menú, elige "Desactivar en este sitio" o "Pausar en este sitio"',
        'Actualiza la página si no se actualiza automáticamente'
      ]
    }
  };

  let browserLanguage = (navigator.language || navigator.userLanguage).substring(0, 2).toLowerCase();
  // Check if the detected language is supported (exists in localizedTexts)
  if (!Object.keys(localizedTexts).includes(browserLanguage)) {
  ...
  const config = {
    ...
    iconColor: "rgb(254 75 255)", // Icon color of popup (Please note there are 3 icons already present in the file (handIcon, adIcon, faceIcon) you can use any of the icons, just change icon name in constructModal function)
    ...
    ...localizedTexts[browserLanguage],
